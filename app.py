...
novels_db = {}
characters_db = {} # New: In-memory storage for characters
...
def create_novel():
    ...
    new_novel = {
        ...
        "characters": "", # Initialize metadata fields
        "glossary": "",
        "knowledge": ""
...
def get_novel(novel_id):
    ...
    if novel:
        return jsonify({
            ...
            "characters": novel.get("characters", ""), # Return metadata
...
def update_novel(novel_id):
    ...
    if 'glossary' in data:
        novels_db[novel_id]['glossary'] = data['glossary']
...
def export_novel_serverside(novel_id, file_format):
    ...
    try:
        if file_format == 'txt':
            ...
            if glossary: txt_content += f"词条设定:\n{glossary}\n\n"
            ...
            if characters:
                ...
                document.add_paragraph()
            if glossary:
                document.add_heading('词条设定', level=2)
...
def get_characters():
    ...
    # Return characters as a list of dictionaries including their IDs
    char_list = [{"id": char_id, **data} for char_id, data in characters_db.items()]
...
def create_character():
    ...
    characters_db[char_id] = new_char
    print(f"Created character: {char_id} - {data['name']}")
    return jsonify({"id": char_id, **new_char}), 201
...
def get_character(char_id):
    ...
    if character:
        return jsonify({"id": char_id, **character})
    else:
        return jsonify({"error": "Character not found"}), 404

@app.route('/api/characters/<char_id>', methods=['PUT'])
def update_character(char_id):
    """API endpoint to update an existing character."""
    ...
    if 'name' in data:
        characters_db[char_id]['name'] = data['name']
    if 'description' in data:
        characters_db[char_id]['description'] = data['description']

    print(f"Updated character: {char_id}")
    return jsonify({"message": "Character updated successfully", "id": char_id, **characters_db[char_id]})
...
def delete_character(char_id):
    ...
    if char_id in characters_db:
        deleted_name = characters_db[char_id].get('name', 'Unnamed')
        del characters_db[char_id]
        print(f"Deleted character: {char_id} - {deleted_name}")
        return jsonify({"message": f"Character '{deleted_name}' deleted successfully"})
    else:
        return jsonify({"error": "Character not found"}), 404
